# coding = utf-8
"""
总结通过位运算实现base64编码和解码的核心算法，并运用AOP思想，使用类装饰器，分析算法实现的正确性。
...
base64解码核心算法：
    1、根据字节，找到base64索引，通过移位运算，重新组成3字节的24bit的，在通过大端模式，转换成bytes。
    2、补0处理：编码中，末尾补0被替换成=号，将无法在base64编码表中找到索引数字，再这里判断是否需要移位运算。
...
class Base64Demo(object):
    ...
    def bit_decode_base64(self, src: bytes):
        """
        移位运算解码
        4个字节一组，找到对应的base64索引，重新组成3字节的24bit的，在通过大端模式，转换成bytes，末尾的=strip(b'\x00')
        """
        res = bytearray()
