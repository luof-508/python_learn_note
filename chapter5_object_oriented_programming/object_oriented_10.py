...
创建与销毁的对象模型：
`__new__`构建实例，不管什么语言，一般不要乱动。构建实例，无非就是类是一个模板，利用这个模板在内存中创建出一个跟类相关的实例，
而这个实例会在内存中占一块地方用于存储类的相关属性，python存放相关属性用字典管理`__dict__`，从而建立了一个名称与对象的映射关系，相当于构建了一张表方便查询。
实例放在内存中，它的属性值在内存中是散落的，建立字典`__dict__`便于管理或者统一管理。
实例构建完之后，马上进行初始化`__init__`，因为`__new__`只是照着类模板构建了实例，但是实例还没有自己个性化的属性，怎么办，添呗，就用初始化函数`__init__`，
把个性化的东西放在实例的`__dict__`中，方便查阅，实际上在内存中也是散落的。
`__del__`，对象销毁的时候调用，实际用处很少
